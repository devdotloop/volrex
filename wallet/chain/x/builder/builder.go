...
func (b *builder) NewBaseTx(
	...
	toBurn := map[ids.ID]uint64{
		b.context.VOLREXAssetID: b.context.BaseTxFee,
	}
...
func (b *builder) NewCreateAssetTx(
	...
	toBurn := map[ids.ID]uint64{
		b.context.VOLREXAssetID: b.context.CreateAssetTxFee,
	}
	ops := common.NewOptions(options)
...
func (b *builder) NewImportTx(
	...
	var (
		...
		importedAVAX = importedAmounts[volrexAssetID]
	)
	if importedAVAX > txFee {
		importedAmounts[volrexAssetID] -= txFee
	} else {
		if importedAVAX < txFee { // imported amount goes toward paying tx fee
			toBurn := map[ids.ID]uint64{
				volrexAssetID: txFee - importedAVAX,
			}
			var err error
			inputs, outputs, err = b.spend(toBurn, ops)
			if err != nil {
		...
		delete(importedAmounts, volrexAssetID)
	}
