...
var (
	...
	// We hard-code [volrexAssetID] and [subnetAssetID] to make ordering of UTXOs
	// generated by [makeTestUTXOs] reproducible.
	volrexAssetID = ids.Empty.Prefix(1789)
...
func TestAddPermissionlessValidatorTx(t *testing.T) {
	...
	var (
		utxos = []*avax.UTXO{
			makeUTXO(1 * units.NanoVolrex), // small UTXO
			makeUTXO(9 * units.Volrex),     // large UTXO
		}
	...
	for _, e := range testEnvironment {
		t.Run(e.name, func(t *testing.T) {
			...
			require.Equal(
				map[ids.ID]uint64{
					volrexAssetID: primaryNetworkPermissionlessStaker.Wght,
				},
	...
	for _, e := range testEnvironment {
		t.Run(e.name, func(t *testing.T) {
			...
			requireFeeIsCorrect(
				...
				nil,
				map[ids.ID]uint64{
					e.context.VOLREXAssetID: 3 * units.Volrex, // Balance of the validators
				},
			)
		})
	}
	...
	for _, e := range testEnvironment {
		t.Run(e.name, func(t *testing.T) {
			...
			requireFeeIsCorrect(
				...
				nil,
				map[ids.ID]uint64{
					e.context.VOLREXAssetID: balance, // Balance of the validator
				},
			)
		})
	}
	...
	for _, e := range testEnvironment {
		t.Run(e.name, func(t *testing.T) {
			...
			requireFeeIsCorrect(
				...
				nil,
				map[ids.ID]uint64{
					e.context.VOLREXAssetID: balance, // Balance increase
				},
			)
		})
	}
