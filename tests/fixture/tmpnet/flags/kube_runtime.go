...
func (v *kubeRuntimeVars) registerWithFlag() {
	v.config = &KubeconfigVars{}
	v.config.RegisterWithFlag(kubeDocPrefix)
	v.register(flag.StringVar)
}

func (v *kubeRuntimeVars) registerWithFlagSet(flagSet *pflag.FlagSet) {
	v.config = &KubeconfigVars{}
	v.config.RegisterWithFlagSet(flagSet, kubeDocPrefix)
	v.register(flagSet.StringVar)
}

func (v *kubeRuntimeVars) register(stringVar stringVarFunc) {
	stringVar(
		&v.namespace,
		"kube-namespace",
