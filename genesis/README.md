...
- `volrexAddr`: X/P Chain address to receive the allocation
- `initialAmount`: Initial unlocked amount minted to the `volrexAddr` on the X-Chain
- `unlockSchedule`: List of locked, stakeable UTXOs minted to `volrexAddr` on the P-Chain

Note: if an `volrexAddr` from allocations is included in `initialStakers`, the genesis includes
